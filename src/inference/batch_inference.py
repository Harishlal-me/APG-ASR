...
def process_single_grid(raw_img, img_name, gt_boxes, gt_classes, active_detector, model_label, output_dir, main_title):
    ...
    composite = create_2x2_composite(panel_a, panel_b, panel_c, panel_d, dynamic_title)
    out_path = os.path.join(output_dir, f"grid_{model_label}_{img_name}")
    cv2.imwrite(out_path, composite)
    
    if best_improvement_val >= 0.05:
        improved_dir = os.path.join(output_dir, "improved")
        os.makedirs(improved_dir, exist_ok=True)
        improved_path = os.path.join(improved_dir, f"grid_{model_label}_iou+{best_improvement_val:.2f}_{img_name}")
        cv2.imwrite(improved_path, composite)
...
def run_batch(input_dir, output_dir, main_title):
    ...
    for img_path in tqdm(image_paths):
        ...
        # 1. Generate PRETRAINED Grid
        if pretrained_detector:
            pretrained_out = os.path.join(output_dir, "pretrained_grids")
            os.makedirs(pretrained_out, exist_ok=True)
            process_single_grid(raw_img, img_name, gt_boxes, gt_classes, pretrained_detector, "Pretrained", pretrained_out, main_title)
            
        # 2. Generate CUSTOM Grid
        if custom_detector:
            custom_out = os.path.join(output_dir, "custom_grids")
            os.makedirs(custom_out, exist_ok=True)
            process_single_grid(raw_img, img_name, gt_boxes, gt_classes, custom_detector, "Custom", custom_out, main_title)
